    ...
    return html`
      <div class="cr-provider-picker relative flex flex-col gap-2">
        <div class="flex items-center gap-2">
          <button
            type="button"
            class="cr-provider-picker__trigger btn btn-sm min-h-11 flex-1 items-center justify-between gap-3 rounded-[0.7rem] border border-base-100/10 bg-base-200/80 px-3 text-left shadow-sm"
          ...
          <button
            type="button"
            class="btn btn-sm btn-ghost min-h-11 rounded-[0.7rem] border border-base-100/10 bg-base-200/72 px-3"
        ...
        ${this.error
          ? html`<div class="rounded-[0.7rem] border border-error/20 bg-error/10 px-3 py-2 text-xs text-error-content">${this.error}</div>`
          : ""}

        ${this.open
          ? html`
              <div class="cr-provider-picker__panel absolute left-0 right-0 top-full z-20 mt-1.5 rounded-[0.9rem] border border-base-100/10 bg-base-100/98 p-3 shadow-2xl backdrop-blur-md">
                <label class="input input-sm flex h-10 items-center gap-2 rounded-[0.65rem] border border-base-100/10 bg-base-200/82 px-3">
                ...
                <div class="mt-2 max-h-72 overflow-y-auto pr-1">
                  ${this.filteredOptions.length > 0
                    ? html`
                        <div class="flex flex-col gap-1.5">
                          ${this.filteredOptions.map(
                            (option) => html`
                              <button
                                type="button"
                                class="flex w-full flex-col gap-1 rounded-[0.75rem] border px-3 py-2.5 text-left transition-colors ${option.id === this.selectedId
                                  ? "border-primary/35 bg-primary/10"
                                  : "border-base-100/10 bg-base-200/50 hover:border-primary/22 hover:bg-base-200"}"
                      ...
                      `
                    : html`
                        <div class="rounded-[0.75rem] border border-dashed border-base-100/10 bg-base-200/45 px-4 py-6 text-center text-sm text-base-content/55">
