# @cr/desktop — Desktop App (Electrobun)

## Overview
Native desktop wrapper for the CR web dashboard. Uses Electrobun to run the
existing `@cr/server` + `@cr/web` stack inside a native window with system
webview.

## Architecture
- **Main process** (`src/bun/index.ts`): Starts an embedded `@cr/server` on an
...
- **App state** (`src/bun/app-state.ts`): SQLite database at `~/.cr/desktop.db`
...
- Zero modifications to `@cr/web` or `@cr/server` — all desktop logic is
